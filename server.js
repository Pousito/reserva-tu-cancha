...
// Endpoint para verificar y corregir esquema de producción
app.get('/api/check-schema', async (req, res) => {
    try {
        console.log('🔍 VERIFICANDO ESQUEMA DE PRODUCCIÓN');
        ...
        for (const columna of columnasAAgregar) {
            if (!columnasExistentes.includes(columna.nombre)) {
                try {
                    ...
                    await db.query(`ALTER TABLE reservas ADD COLUMN ${columna.nombre} ${columna.tipo};`);
                    columnasAgregadas.push(columna.nombre);
        ...
        // Verificación final
        const estructuraFinal = await db.query(`
            SELECT column_name, data_type, is_nullable, column_default
            FROM information_schema.columns 
            WHERE table_name = 'reservas' 
            ORDER BY ordinal_position;
