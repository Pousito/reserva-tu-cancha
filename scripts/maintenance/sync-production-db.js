 ...
 * 
 * Este script debe ejecutarse en el entorno de producción para dejar la base de datos con:
...
async function syncProductionDatabase() {
    ...
    try {
        console.log('🔄 Iniciando sincronización de base de datos de producción...');
        ...
        let ciudadLosAngeles = await db.get('SELECT id FROM ciudades WHERE nombre = $1', ['Los Ángeles']);
        if (!ciudadLosAngeles) {
            console.log('🏙️ Creando ciudad Los Ángeles...');
            const result = await db.run('INSERT INTO ciudades (nombre) VALUES ($1)', ['Los Ángeles']);
        ...
        let magnasports = await db.get('SELECT id FROM complejos WHERE nombre = $1', ['MagnaSports']);
        if (!magnasports) {
        ...
        console.log('\n🎉 Sincronización de producción completada exitosamente');
