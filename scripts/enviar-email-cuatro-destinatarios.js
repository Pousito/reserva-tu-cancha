 ...
 * a 4 destinatarios, con el email de Bastián en el contenido
 */

const nodemailer = require('nodemailer');
require('dotenv').config();

async function enviarEmailCuatroDestinatarios() {
    try {
        ...
        const cancha = 'Cancha Principal';
        const emailBastian = 'eliecer.castillo.cabrera@gmail.com'; // Email de Bastián para el contenido
        
        // Lista de destinatarios (quienes recibirán el email)
        const destinatarios = [
            'naxiin320@gmail.com',
            'ignacio.araya.lillo@gmail.com',
            'eliecer.castillo.cabrera@gmail.com',
            'magda.espinoza.se@gmail.com'
        ];
    ...
    <div style="background-color: white; padding: 30px; border-radius: 10px; box-shadow: 0 0 10px rgba(0,0,0,0.1);">
        ...
        <div style="background-color: #f8f9fa; padding: 20px; border-radius: 8px; margin: 20px 0;">
            ...
            <div style="display: flex; justify-content: space-between; margin: 10px 0; padding: 8px 0; border-bottom: 1px solid #e9ecef;">
                ...
                <span style="color: #212529;">${emailBastian}</span>
            </div>
            <div style="display: flex; justify-content: space-between; margin: 10px 0; padding: 8px 0; border-bottom: 1px solid #e9ecef;">
...
Gracias por elegir Reserva Tu Cancha!
        ...
        console.log('\n📧 Enviando emails a 4 destinatarios...');
        console.log('Destinatarios:');
        destinatarios.forEach((email, index) => {
            console.log(`   ${index + 1}. ${email}`);
        });
        console.log(`\nEmail en el contenido: ${emailBastian} (Bastián)\n`);
        ...
        console.log('\n📋 Detalles del email:');
        console.log(`   - Email en contenido: ${emailBastian} (Bastián)`);
        console.log(`   - Cancha: ${cancha}`);
        console.log(`   - Complejo: ${complejo}`);
        console.log(`   - Fecha: ${fecha}`);
        console.log(`   - Horario: ${horaInicio} - ${horaFin}`);
        console.log(`   - Total enviados: ${resultados.filter(r => r.success).length} de ${destinatarios.length}`);
...
enviarEmailCuatroDestinatarios()
    .then(() => {
        console.log('\n✅ Proceso completado');
        process.exit(0);
    })
