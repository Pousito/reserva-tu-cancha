...
/**
 * Pruebas Simplificadas de Reservas y Bloqueos Temporales
 * 
 * Este script ejecuta las pruebas usando solo las APIs del servidor,
...
class SimpleReservationTester {
  ...
  async initialize() {
    console.log('🚀 INICIANDO PRUEBAS SIMPLIFICADAS DE RESERVAS');
    console.log('==============================================');
  ...
  async createReservationFromWeb() {
    try {
      ...
      // Simular selección de datos básicos
      console.log('🏙️ Simulando selección de datos...');
      await this.pages.web.evaluate(() => {
        // Simular que se han seleccionado los datos
        window.testData = {
          ciudad: '1',
          complejo: '1',
          tipo: 'padel',
      ...
      // Crear bloqueo temporal directamente via API
      ...
      const bloqueoResponse = await this.pages.web.evaluate(async () => {
        const response = await fetch('/api/reservas/bloquear', {
          ...
          body: JSON.stringify({
            cancha_id: 1,
            fecha: window.testData.fecha,
            hora_inicio: window.testData.hora,
            hora_fin: '15:00',
...
if (require.main === module) {
  const tester = new SimpleReservationTester();
  tester.runAllTests().catch(console.error);
}

module.exports = SimpleReservationTester;
