...
/**
 * Pruebas Finales de Reservas y Bloqueos Temporales
 * 
 * Este script ejecuta las pruebas básicas para verificar que el sistema
 * funciona correctamente antes de llevarlo a producción
 */
...
class FinalReservationTester {
  ...
  async initialize() {
    console.log('🚀 INICIANDO PRUEBAS FINALES DE RESERVAS');
    console.log('========================================');
  ...
  async createReservationFromWeb() {
    try {
      ...
      const response = await this.makeRequest('POST', '/api/reservas/bloquear-y-pagar', {
        ...
        hora_inicio: hora,
        hora_fin: this.getNextHour(hora),
        precio_total: 25000,
...
if (require.main === module) {
  const tester = new FinalReservationTester();
  tester.runAllTests().catch(console.error);
}

module.exports = FinalReservationTester;
