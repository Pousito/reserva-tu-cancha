...
/**
 * 🔍 BÚSQUEDA DE RESERVAS COMO LA APLICACIÓN
 * 
 * Este script usa la misma lógica que la aplicación web
...
class BusquedaReservasComoApp {
    ...
    async buscarReservasComoApp() {
        console.log('\n🔍 BUSCANDO RESERVAS COMO LA APLICACIÓN...');
        console.log('=' .repeat(60));
        
        try {
            // Usar la misma lógica que la aplicación web
            const query = `
                SELECT 
                    r.id,
                    r.codigo_reserva,
    ...
    async buscar() {
        console.log('🔍 BÚSQUEDA DE RESERVAS COMO LA APLICACIÓN');
        console.log('=' .repeat(60));
        console.log(`📋 Reservas a buscar: ${this.reservasEspecificas.join(', ')}`);
        
        await this.conectar();
        
        // 1. Buscar reservas como la app
        const reservas = await this.buscarReservasComoApp();
...
if (require.main === module) {
    const busqueda = new BusquedaReservasComoApp();
    busqueda.buscar().catch(console.error);
}

module.exports = BusquedaReservasComoApp;
