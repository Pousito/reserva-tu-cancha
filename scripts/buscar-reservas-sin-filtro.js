...
/**
 * 🔍 BÚSQUEDA DE RESERVAS SIN FILTRO DE COMPLEJO
 * 
 * Este script busca las reservas BQNI8W, IJRGBH y 1XJAKD
 * sin aplicar filtros de complejo, como lo haría un super_admin
 */
...
class BusquedaReservasSinFiltro {
    constructor() {
        this.pool = null;
    ...
    async buscarReservasSinFiltro() {
        console.log('\n🔍 BUSCANDO RESERVAS SIN FILTRO DE COMPLEJO...');
        console.log('=' .repeat(60));
        
        try {
            // Usar la misma consulta que la aplicación pero SIN filtro de complejo
            const query = `
            ...
            const reservas = await this.pool.query(query, [this.reservasEspecificas]);
    ...
    async buscar() {
        console.log('🔍 BÚSQUEDA DE RESERVAS SIN FILTRO DE COMPLEJO');
        console.log('=' .repeat(60));
        console.log(`📋 Reservas a buscar: ${this.reservasEspecificas.join(', ')}`);
        
        await this.conectar();
        
        // 1. Buscar reservas sin filtro
        const reservas = await this.buscarReservasSinFiltro();
...
if (require.main === module) {
    const busqueda = new BusquedaReservasSinFiltro();
    busqueda.buscar().catch(console.error);
}

module.exports = BusquedaReservasSinFiltro;
