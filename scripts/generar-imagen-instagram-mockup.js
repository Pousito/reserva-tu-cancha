...
function generarImagenInstagram() {
    const doc = new jsPDF('landscape', 'mm', [210, 148]); // Tamaño Instagram

    // Fondo de césped deportivo (verde sólido)
    doc.setFillColor('#4a7c59'); // Verde medio
    doc.rect(0, 0, 210, 148, 'F');

    // Agregar textura de césped (líneas sutiles)
    doc.setDrawColor('#1e3a1e');
    doc.setLineWidth(0.3);
    for (let i = 0; i < 200; i += 8) {
        if (i < 200) {
            doc.line(i, 0, i + 5, 148);
        }
    }
    ...
    doc.roundedRect(iphoneX, iphoneY, iphoneWidth, iphoneHeight, 8, 'F');

    // Pantalla del iPhone
    doc.setFillColor('#000000');
    doc.roundedRect(iphoneX + 3, iphoneY + 8, iphoneWidth - 6, iphoneHeight - 16, 5, 'F');

    // Contenido de la pantalla móvil (simplificado)
    doc.setFillColor('#6b46c1'); // Morado
    ...
    // Botón móvil
    doc.setFillColor('#ffffff');
    doc.roundedRect(iphoneX + 8, iphoneY + 55, iphoneWidth - 16, 8, 3, 'F');
    doc.setFontSize(7);
    ...
    doc.roundedRect(macbookX, macbookY + 35, macbookWidth, 8, 2, 'F');

    // Pantalla del MacBook
    doc.setFillColor('#1f2937');
    doc.roundedRect(macbookX + 2, macbookY, macbookWidth - 4, macbookHeight, 3, 'F');

    // Sidebar del admin
    doc.setFillColor('#4c1d95'); // Morado oscuro
    ...
    doc.setFillColor('#dcfce7'); // Verde claro
    ...
    doc.roundedRect(macbookX + 45, macbookY + 38, 12, 4, 1, 'F');
    doc.setFontSize(4);
    doc.setTextColor('#ffffff');
